USE ROLE accountadmin;
CREATE DATABASE course_repo;
USE SCHEMA public;

-- Create credentials
CREATE OR REPLACE SECRET course_repo.public.github_pat
  TYPE = password
  USERNAME = 'FranciscoRossi48'
  PASSWORD = '';

-- Create the API integration
CREATE OR REPLACE API INTEGRATION git_https_api
  API_PROVIDER = git_https_api
  API_ALLOWED_PREFIXES = ('https://github.com/FranciscoRossi48/')
  ALLOWED_AUTHENTICATION_SECRETS = ('course_repo.public.github_pat')
  ENABLED = TRUE;

-- Create the git repository object
CREATE OR REPLACE GIT REPOSITORY course_repo.public.advanced_data_engineering_snowflake
  API_INTEGRATION = git_https_api
  ORIGIN = 'https://github.com/FranciscoRossi48/advanced-data-engineering-snowflake'
  GIT_CREDENTIALS = course_repo.public.github_pat;

-- List the git repositories
SHOW GIT REPOSITORIES;